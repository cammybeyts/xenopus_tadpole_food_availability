...
# ORCID ID: XX                                                        #
# Email: XX                                                           #
# Date: First created Feb 2023.  Last modified 18 Dec 2024            #
#---------------------------------------------------------------------#

#script naming convention for ecological context/assays and treatment names:
##ACT = familiar context / activity assay
##EXP = unfamiliar context / exploration assay
...
noegg_model_summary <- summary(noegg_model)
...
full.data1 <- read.data %>%
  ...
  mutate(Rep_recode = recode(Rep, 0, 1, 2, 3, 4, 5, 6, 7)) %>%
  # reps 12345678 recoded to 01234567 %>%
  mutate(RepCon = as.numeric(Rep)) %>%
  mutate(RepCon_recode = as.numeric(Rep_recode))


full.data_suummary <- full.data1 %>%
  group_by(Treatment) %>%
  count(TadpoleID)
...
write.csv(noegg_pd, "noegg_pd.csv")
